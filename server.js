import express from "express";
import multer from "multer";
import sharp from "sharp";
import { removeBackground } from "@imgly/background-removal-node";

const app = express();
const upload = multer({ storage: multer.memoryStorage(), limits: { fileSize: 20 * 1024 * 1024 } });

app.use(express.static("public"));

async function enhancePngBuffer(pngBuffer) {
  const rgbaObj = await sharp(pngBuffer)
    .ensureAlpha()
    .raw()
    .toBuffer({ resolveWithObject: true });

  const { width, height, channels } = rgbaObj.info;
  if (channels !== 4) {
    return sharp(pngBuffer)
      .ensureAlpha()
      .modulate({ saturation: 1.10, brightness: 1.02 })
      .sharpen(0.4)
      .png({ compressionLevel: 9 })
      .toBuffer();
  }

  const rgba = rgbaObj.data;
  const pixelCount = width * height;

  const rgb = Buffer.allocUnsafe(pixelCount * 3);
  const a = Buffer.allocUnsafe(pixelCount);

  for (let i = 0, j = 0, k = 0; i < rgba.length; i += 4) {
    rgb[j++] = rgba[i];
    rgb[j++] = rgba[i + 1];
    rgb[j++] = rgba[i + 2];
    a[k++] = rgba[i + 3];
  }

  const enhancedRgbObj = await sharp(rgb, { raw: { width, height, channels: 3 } })
    .modulate({ saturation: 1.15, brightness: 1.02 })
    .linear(1.03, -1.5)
    .sharpen(0.6)
    .raw()
    .toBuffer({ resolveWithObject: true });

  return sharp(enhancedRgbObj.data, { raw: { width, height, channels: 3 } })
    .joinChannel(a, { raw: { width, height, channels: 1 } })
    .png({ compressionLevel: 9 })
    .toBuffer();
}

async function findAlphaBBox(pngBuffer, alphaThreshold = 10) {
  const img = sharp(pngBuffer).ensureAlpha();
  const { data, info } = await img.raw().toBuffer({ resolveWithObject: true });

  const w = info.width;
  const h = info.height;
  const channels = info.channels;

  let left = w,
    top = h,
    right = -1,
    bottom = -1;

  for (let y = 0; y < h; y++) {
    const rowStart = y * w * channels;
    for (let x = 0; x < w; x++) {
      const idx = rowStart + x * channels;
      const a = data[idx + 3];
      if (a > alphaThreshold) {
        if (x < left) left = x;
        if (y < top) top = y;
        if (x > right) right = x;
        if (y > bottom) bottom = y;
      }
    }
  }

  if (right === -1) return { left: 0, top: 0, right: w - 1, bottom: h - 1 };
  return { left, top, right, bottom };
}

async function autoCropSquare(
  pngBuffer,
  {
    paddingRatio = 0.10,
    outputSize = 1200,
    background = { r: 255, g: 255, b: 255, alpha: 0 }
  } = {}
) {
  const bbox = await findAlphaBBox(pngBuffer);

  const width = bbox.right - bbox.left + 1;
  const height = bbox.bottom - bbox.top + 1;
  const maxSide = Math.max(width, height);

  const pad = Math.round(maxSide * paddingRatio);

  const cropLeft = Math.max(0, bbox.left - pad);
  const cropTop = Math.max(0, bbox.top - pad);

  const cropWidth = width + pad * 2;
  const cropHeight = height + pad * 2;

  const meta = await sharp(pngBuffer).metadata();
  const imgW = meta.width ?? 0;
  const imgH = meta.height ?? 0;

  const safeCropWidth = Math.min(cropWidth, imgW - cropLeft);
  const safeCropHeight = Math.min(cropHeight, imgH - cropTop);

  const cropped = sharp(pngBuffer)
    .extract({ left: cropLeft, top: cropTop, width: safeCropWidth, height: safeCropHeight })
    .ensureAlpha();

  const cropMeta = await cropped.metadata();
  const cW = cropMeta.width ?? safeCropWidth;
  const cH = cropMeta.height ?? safeCropHeight;

  const square = Math.max(cW, cH);
  const extendLeft = Math.floor((square - cW) / 2);
  const extendRight = square - cW - extendLeft;
  const extendTop = Math.floor((square - cH) / 2);
  const extendBottom = square - cH - extendTop;

  return cropped
    .extend({ top: extendTop, bottom: extendBottom, left: extendLeft, right: extendRight, background })
    .resize(outputSize, outputSize, { fit: "contain", background })
    .png({ compressionLevel: 9 })
    .toBuffer();
}

app.post("/process", upload.single("image"), async (req, res) => {
  const t0 = Date.now();
  try {
    console.log("\n--- /process START ---");

    if (!req.file?.buffer || !req.file.mimetype?.startsWith("image/")) {
      return res.status(400).json({ error: "Please upload a valid image file." });
    }

    // Optional controls from the UI (sent as multipart fields)
    const outputSizeRaw = parseInt(req.body?.size || "1200", 10);
    const outputSize = Math.max(300, Math.min(4000, Number.isFinite(outputSizeRaw) ? outputSizeRaw : 1200));

    // enhance=1 => apply our colour enhancement; enhance=0 => keep colours intact
    const enhance = String(req.body?.enhance ?? "1") === "1";

    console.log("Options:", { outputSize, enhance });

    // Downscale to speed up ML background removal
    const inputPng = await sharp(req.file.buffer)
      .rotate()
      .resize({ width: 2200, height: 2200, fit: "inside", withoutEnlargement: true })
      .png()
      .toBuffer();
    console.log("Stage: normalized+downscaled PNG in", Date.now() - t0, "ms");

    const removalConfig = {
      model: "medium",
      output: { format: "image/png", quality: 0.95 }
    };

    if (!globalThis.Blob) {
      throw new Error("Global Blob is not available in this Node runtime. Please use Node 18+ / 20+.");
    }

    res.setTimeout(240000);

    const inputBlob = new Blob([inputPng], { type: "image/png" });
    const bgRemoved = await Promise.race([
      removeBackground(inputBlob, removalConfig),
      new Promise((_, reject) => setTimeout(() => reject(new Error("Background removal timed out")), 180000))
    ]);
    console.log("Stage: background removed in", Date.now() - t0, "ms");

    const bgRemovedBuf = Buffer.isBuffer(bgRemoved) ? bgRemoved : Buffer.from(await bgRemoved.arrayBuffer());

    const enhanced = enhance ? await enhancePngBuffer(bgRemovedBuf) : bgRemovedBuf;
    console.log(`Stage: ${enhance ? "enhanced" : "enhance skipped"} in`, Date.now() - t0, "ms");

    const square = await autoCropSquare(enhanced, { paddingRatio: 0.12, outputSize });
    console.log("Stage: auto-cropped square in", Date.now() - t0, "ms");

    console.log("--- /process END ---\n");

    res.json({
      enhancedBase64: enhanced.toString("base64"),
      squareBase64: square.toString("base64"),
      outputSize,
      enhance
    });
  } catch (err) {
    console.log("--- /process ERROR after", Date.now() - t0, "ms ---");
    console.error(err);
    res.status(500).json({ error: "Processing failed", details: String(err?.message || err) });
  }
});

const port = process.env.PORT || 3000;
app.listen(port, "0.0.0.0", () => console.log(`✅ Product Prep running on http://0.0.0.0:${port}`));